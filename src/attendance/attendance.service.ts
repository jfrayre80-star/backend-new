...
export class AttendanceService {
  ...
  private async findActiveSchedule(scheduleId: string): Promise<Schedules> {
    ...
    if (!schedule) {
      throw new NotFoundException("El horario especificado no existe o se encuentra inactivo.");
    }
    return schedule;
  }
  ...
  private async findActiveQr(scheduleId: string): Promise<QrCodes | null> {
    return this.qrCodesRepository.findOne({
      where: { schedule: { id: scheduleId }, isActive: true },
  ...
  async scanQr(userIdFromToken: string, dto: ScanQrDto) {
    ...
    if (!enrollment) {
      throw new ForbiddenException("No estás inscrito en el grupo de este horario.");
    }
  ...
  async closeAttendance(scheduleId: string, recordedByUserId?: string) {
    ...
    const existingRecords = await this.attendanceRecordsRepository.find({
      where: { scheduleId: schedule.id, recordedDate: todayStr },
    });
  ...
  async updateStudentAttendanceManual(dto: ManualAttendanceDto, teacherUserId: string) {
    ...
    if (record) {
      ...
      record.auditTrail = {
        ...(record.auditTrail || {}),
        updatedManuallyAt: new Date().toISOString(),
      ...
      record = this.attendanceRecordsRepository.create({
        ...
        auditTrail: {
          createdManuallyAt: new Date().toISOString(),
  ...
  async getStudentMetrics(
    ...
    if (currentUser.role === "parent") {
      ...
      if (!parent) {
        throw new ForbiddenException("Acceso denegado. No se encontró registro de padre vinculado.");
      }
      ...
      if (!isMyChild) {
        throw new ForbiddenException("Acceso denegado. Este alumno no está vinculado a tu cuenta.");
      }
    }
    ...
    if (currentUser.role === "student") {
      ...
      if (!isSelf) {
        throw new ForbiddenException("Solo puedes consultar tu propio historial de asistencia.");
      }
    }
    ...
    const attendancePercentage =
      effectiveClasses > 0 ? Number(((attendedClasses / effectiveClasses) * 100).toFixed(2)) : 100;
  ...
  async createJustification(dto: CreateJustificationDto) {
    ...
    try {
      ...
      const savedRecords = await queryRunner.manager.save(Justifications, recordsToInsert);
  ...
  async updateJustification(id: string, dto: UpdateJustificationDto) {
    ...
    if (dto.reason) {
      justification.reason = dto.reason;
    }
    if (dto.justificationDate) {
      justification.justificationDate = dto.justificationDate;
    }
    if (dto.registeredBy) {
      justification.registeredBy = { id: dto.registeredBy } as any;
    }
  ...
  async findJustificationsByStudentName(searchTerm: string) {
    ...
    return this.justificationsRepository.find({
      where: [
        {
          student: { user: { firstName: ILike(`%${cleanTerm}%`) } },
          isActive: true,
        },
        {
          student: { user: { lastName: ILike(`%${cleanTerm}%`) } },
          isActive: true,
        },
      ],
      relations: { student: { user: true } },
  ...
  async findAllJustifications(date?: string) {
    ...
    return this.justificationsRepository.find({
      where: whereCondition,
      relations: { student: { user: true } },
      order: { justificationDate: "DESC", createdAt: "DESC" },
    });
  }
